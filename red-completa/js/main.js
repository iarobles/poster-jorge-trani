...
jQuery.getJSON(BASE_PATH + GetQueryStringParams("config", "config.json"), function (data, textStatus, jqXHR) {
    config = data;
...
function initSigma(config) {
    var data = BASE_PATH + config.data;
...
function setupGUI(config) {
    ...
    if (config.logo.file) {
        ...
        logo = "<img src=\"" + BASE_PATH + config.logo.file + "\"";
...
function nodeActive(a) {
    ...
    if (f.attributes) {
        ...
        if (image_attribute) {
            ...
            $GP.info_name.html("<div><img src=" + BASE_PATH + f.attributes[image_attribute] + " style=\"vertical-align:middle\" /> <span onmouseover=\"sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex['" + b.id + '\'])" onmouseout="sigInst.refresh()">' + b.label + "</span></div>");
        } else {
